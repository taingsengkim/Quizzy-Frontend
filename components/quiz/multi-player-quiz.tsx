...
export default function MultiplayerQuizPage({ quizId }: { quizId: string }) {
  ...
  "
          >
            {/* STATUS BADGE */}
            <span
              className={`text-[11px] font-mono font-semibold tracking-wider px-3 py-1 rounded-full border ${
                room.finished
            ...
            <div className="flex items-center flex-wrap gap-2 text-2xl font-mono">
              ...
              <span className="text-sky-600 dark:text-sky-400 uppercase tracking-widest font-semibold">
                {quiz?.title}
              </span>
            </div>
          </div>
            ...
            <div className="bg-gray-50 dark:bg-[#080e1c] border border-gray-200 dark:border-slate-800 rounded-2xl p-4">
                ...
                {room.participants?.map((p) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {p === room.owner && (
                        <span className="text-[20px] text-emerald-600 dark:text-emerald-500 font-extrabold opacity-60">
                          (host)
                        </span>
                      )}
              ...
              {!room.started && isOwner && (
                <button
                  ...
                  className="relative cursor-pointer w-full py-3.5 mt-5 rounded-xl font-mono font-bold text-sm tracking-widest uppercase overflow-hidden group
                ...
                <div className="bg-gray-50 dark:bg-[#080e1c] border border-gray-200 dark:border-slate-800 rounded-2xl p-6 space-y-4 font-mono">
                  <div className="text-center space-y-1">
                    ...
                    <p className="text-gray-400 dark:text-slate-500 text-xs">
                      {finishedCount}/{totalPlayers} players finished — waiting
                      for others...
                    </p>
                  </div>
                  <div>
            ...
            : "bg-amber-500 text-black hover:opacity-90"
              ...
              {room.finished &&
                (() => {
                  ...
                  const correctCount = myHistory.filter(
                    (r) => r.correct,
                  ).length;
